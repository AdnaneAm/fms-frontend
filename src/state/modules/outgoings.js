...
export const actions = {
  ...
  createOutgoing({commit,rootGetters},outgoing){
    // Get the relatedToFarmer value 
    if(outgoing.outgoingType == 'farmer'){
      const relatedToFarmer = rootGetters['expenses/getExpenseByLabel'](outgoing.outgoingLabel).expenseRelatedToFarmers;
      console.log(relatedToFarmer)
    }

    axios.post(process.env.VUE_APP_API_BASE_URL+'outgoings/');
    commit('pushOutgoing',outgoing);
  },
  updateOutgoing({commit},outgoing){
